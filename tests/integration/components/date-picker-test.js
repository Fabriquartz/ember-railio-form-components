...
test('date gets formatted to date string', function(assert) {
  this.set('object', Ember.Object.create({ value: new Date(2015, 0, 1) }));
  this.render(hbs`{{date-picker object=object propertyPath="value"}}`);
...
test('filling in a date works', function(assert) {
  ...
  assert.equal(+this.get('object.value'), +(new Date(2015, 4, 5)));
});
...
test('typing in an empty value', function(assert) {
  ...
  assert.equal($input.val(), '');
  assert.equal(this.get('object.value'), null);
});
...
test('typing in date shorthands', function(assert) {
  ...
  assert.equal(+this.get('object.value'), +(new Date(2015, 4, 15)));
  ...
  assert.equal(+this.get('object.value'), +(new Date(year, 4, 15)));

  fillIn($input, '505');

  assert.equal($input.val(), `05-05-${yearStr}`);
  assert.equal(+this.get('object.value'), +(new Date(year, 4, 5)));

  fillIn($input, '5');

  assert.equal($input.val(), `05-0${month}-${yearStr}`);
  assert.equal(+this.get('object.value'), +(new Date(year, month - 1, 5)));
});
...
test('arrow up increases date by one day', function(assert) {
  ...
  assert.equal(+this.get('object.value'), +(new Date(2015, 0, 2)));

  this.set('object.value', new Date(2015, 0, 31));

  arrowUp($input);

  assert.equal($input.val(), '01-02-15');
  assert.equal(+this.get('object.value'), +(new Date(2015, 1, 1)));
});

test('shift + arrow up increases date by one month', function(assert) {
  this.set('object', Ember.Object.create({ value: new Date(2015, 0) }));
  this.render(hbs`{{date-picker object=object propertyPath="value"}}`);
  ...
  this.set('object.value', new Date(2015, 11));

  arrowUp($input, true);

  assert.equal($input.val(), '01-01-16');
  assert.equal(+this.get('object.value'), +(new Date(2016, 0, 1)));
});

test('arrow down decreases date by one day', function(assert) {
  this.set('object', Ember.Object.create({ value: new Date(2015, 0, 2) }));
  this.render(hbs`{{date-picker object=object propertyPath="value"}}`);
  ...
  assert.equal($input.val(), '01-01-15');
  assert.equal(+this.get('object.value'), +(new Date(2015, 0, 1)));

  this.set('object.value', new Date(2015, 1, 1));

  arrowDown($input);

  assert.equal($input.val(), '31-01-15');
  assert.equal(+this.get('object.value'), +(new Date(2015, 0, 31)));
});

test('shift + arrow month decreases date by one month', function(assert) {
  this.set('object', Ember.Object.create({ value: new Date(2015, 1) }));
  this.render(hbs`{{date-picker object=object propertyPath="value"}}`);
  ...
  assert.equal($input.val(), '01-01-15');
  assert.equal(+this.get('object.value'), +(new Date(2015, 0)));

  this.set('object.value', new Date(2015, 0));

  arrowDown($input, true);

  assert.equal($input.val(), '01-12-14');
  assert.equal(+this.get('object.value'), +(new Date(2014, 11)));
});
