...
module('Integration | Component | {{radio-select}}', function (hooks) {
  ...
  test('shows options', async function (assert) {
    ...
    assert.equal(
      ...
      'Option 1',
      'shows option 1 value'
    );
    assert.equal(
      $options[1].textContent.trim(),
      'Option 2',
      'shows option 2 value'
    );
  });
  ...
  test('select value calls updated action', async function (assert) {
    ...
    this.actions.updated = function (value) {
      assert.equal(
        value,
        selectValue,
        'calls update action with clicked value'
      );
    };
  ...
  test('works with form-field wrapper', async function (assert) {
    ...
    this.actions.update = function (object, propertyPath, value) {
      assert.equal(
        value,
        secondValue,
        'calls update action with clicekd value'
      );
  ...
  test('cycle true: shows only selection and changes on click', async function (assert) {
    ...
    assert.true($options.eq(0).hasClass('radio-select__option--empty'));
