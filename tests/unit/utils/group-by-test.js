...
test('group content by groupPath', function(assert) {
  ...
  assert.deepEqual(result[0].options, [item1, item5],
                   'group A has correct items in options');
  assert.deepEqual(result[1].options, [item2, item4],
                   'group C has correct items in options');
  assert.deepEqual(result[2].options, [item3],
                   'group B has correct items in options');
});
